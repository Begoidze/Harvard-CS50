...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(3):
            for j in range(3):
                currentCell = (startCell[0] + i, startCell[1] + j)

                if not (0 <= currentCell[0] < self.height and 0 <= currentCell[1] < self.width):
                    continue

                if currentCell == cell:
                    continue

                if currentCell in self.safes:
                    continue
                if currentCell in self.mines:
                    currentCount -= 1
                else:
                    neighbors.add(currentCell)

        new_sentence = Sentence(neighbors, currentCount)
